...
try:
    ...
    # ionixx1
    DATABASES = {'default': {'ENGINE': 'django.db.backends.postgresql_psycopg2', 'NAME': 'portal_stage',
...
try:
    ...
    # #changing User Table
    # cur.execute("update public.user set email ='@espresso.com' where email = '';")
    # conn.commit()
    #
    # cur.execute("update public.user set email =CONCAT(username,email) where email = '@espresso.com';")
    # conn.commit()
    ...
    # Company
    cur.execute(
        """copy company(id,name,external_id,default_currency,website,employee_count,accounting_type,auth_key,secret_key) from '%s/SELECT_t___FROM_public_company_t.csv' delimiter ',' CSV HEADER;
        """ % base_path)
    conn.commit()
